...
export default {
  ...
  methods: {
    getDataApiOrderByTitle() {
      axios
        .get(
          "https://gateway.marvel.com:443/v1/public/comics?orderBy=title&ts=" +
        ...
        .then(response => {
          this.data_order_title = response.data;
          this.order_results = this.data_order_title.data.results;
        })
        .catch(error => {
...
.orderbytitle > .view-orderbytitle {
  display: flex;
  flex-wrap: wrap;
