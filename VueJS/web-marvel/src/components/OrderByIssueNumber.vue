<template>
  <div class="orderbyissuenumber">
    ...
    <div class="view-orderbyissuenumber">
...
export default {
  ...
  methods: {
    getDataApiOrderByIssueNumber() {
      axios
        .get(
          "https://gateway.marvel.com:443/v1/public/comics?orderBy=issueNumber&ts=" +
        ...
        .then(response => {
          this.data_order_issue_number = response.data;
          this.order_results = this.data_order_issue_number.data.results;
        })
        .catch(error => {
  ...
  mounted: function() {
    this.getDataApiOrderByIssueNumber();
  }
};
</script>
...
.orderbyissuenumber > .view-orderbyissuenumber {
  display: flex;
  flex-wrap: wrap;
